...
public class EntityCulling extends Module {
    ...
    public EntityCulling() {
        super("module.render.EntityCulling", Category.MODS, "Culls entities that are out of range");
...
//        EntityLivingBase entity = e.getEntity();
        ...
        if (EntityCulling.shouldPerformCulling) {
            ...
            switch (cullingMode.get()) {
                case "Grouped":
                    if (entityDistance > entityCullingDis.get()) {
                        e.cancel();
                    }
                    break;
                case "Custom":
                    if (entity instanceof IMob && entityDistance > mobCullingDis.get()) {
                        e.cancel();
                        //passive entity render distance
                    } else if ((entity instanceof EntityAnimal || entity instanceof EntityAmbientCreature || entity instanceof EntityWaterMob) && entityDistance > passiveCullingDis.get()) {
                        e.cancel();
                        //player entity render distance
                    } else if (entity instanceof EntityPlayer && entityDistance > playerCullingDis.get()) {
                        e.cancel();
                    }
                    break;
            }
    ...
    private void check() {
        ...
        switch (cullingDelay.get().intValue() - 1) {
            case 0: {
                delay = 10;
                break;
            }
